...
namespace HL::System::Text
{
	namespace StringFunction
	{
		...
		static size_t StringLength(const CharT* string)noexcept {
			...
			if (string == nullptr)
				return cnt;
			for (; *string != CharacterSet::CharBase<CharT>::EndOfString; string++, cnt++);
			return cnt;
		}
		template<class CharT>
		...
		static inline bool IsLower(CharT c)noexcept {
			return c >= CharacterSet::CharBase<CharT>::LowerA && c <= CharacterSet::CharBase<CharT>::LowerZ;
		}
		template<class CharT>
		static inline bool IsUpper(CharT c)noexcept {
			return c >= CharacterSet::CharBase<CharT>::UpperA && c <= CharacterSet::CharBase<CharT>::UpperZ;
		}
		template<class CharT>
		static inline CharT UpperDelta()noexcept {
			return CharacterSet::CharBase<CharT>::UpperA - CharacterSet::CharBase<CharT>::LowerA;
		}
		template<class CharT>
		...
		static inline bool IsDigit(CharT c)noexcept {
			return (c >= CharacterSet::CharBase<CharT>::DigitZero && c <= CharacterSet::CharBase<CharT>::DigitNine);
		}
		template<class CharT>
		...
		void StringToUpper(CharT* target_string)noexcept {
			while (*target_string != CharacterSet::CharBase<CharT>::EndOfString)
		...
		static FloatT StringToFloat(CharT const* string, size_t count) {
			...
			for (size_t i = iter; i < count; ++i)
			{
				...
				{
					if (!dot_sign)
						int_part = int_part * 10 + (FloatT)(string[i] - CharacterSet::CharBase<CharT>::DigitZero);
					else
					{
						float_part += (FloatT)(string[i] - CharacterSet::CharBase<CharT>::DigitZero) / pow_iter;
						pow_iter *= 10;
					}
				}
				else
		...
		static size_t HexToInt(CharT ch) {
			short type = IsHex(ch);
			if (type == 1)
				return ch - CharacterSet::CharBase<CharT>::LowerA + 10;
			else if (type == 2)
				return ch - CharacterSet::CharBase<CharT>::UpperA + 10;
			else
				return ch - CharacterSet::CharBase<CharT>::DigitZero;
		}
